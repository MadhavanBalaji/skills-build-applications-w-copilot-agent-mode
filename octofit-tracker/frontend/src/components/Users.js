...
const endpointBase = process.env.REACT_APP_CODESPACE_NAME
  ...
  : '';
const apiUrl = 'https://-8000.app.github.dev/api/users';

function Users() {
  const endpoint = `${endpointBase}/api/users/`;
  ...
  const renderTable = (items) => {
    if (items.length === 0) return <div className="alert alert-secondary">No users found.</div>;
